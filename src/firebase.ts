...
export function articleToFirestoreDoc(article: Article): Record<string, any> {
  ...
  return {
    ...
    sectionPage: article.sectionPage || '',
    pageNumber: typeof article.pageNumber === 'number' ? article.pageNumber : undefined,
    paperAssigned: typeof article.pageNumber === 'number',
...
export function firestoreDocToArticle(docData: any, docId: string): Article {
  ...
  return {
    ...
    pageNumber: typeof docData.pageNumber === 'number' ? docData.pageNumber : undefined,
    ...(docData.paperAssigned === true ? { paperAssigned: true } : {}),
...
export async function fetchArticleByIdFromFirestore(articleId: string): Promise<Article | null> {
  ...
  try {
    // Direct document key lookup in Firestore
...
export function subscribeToFirestoreArticles(
  ...
  return onSnapshot(q, (snapshot) => {
    ...
    console.warn('[FIRESTORE] subscription error:', err?.message || err);
